...
// TIMEOUTS
#define ERROR_LED_BLINK_INTERVAL    1000  // * 512 micros
#define TIMEOUT_3_SECONDS                  6000 // * 512 micros
#define TIMEOUT_5_SECONDS                 10000 // * 512 micros

// DIGITAL OUTPUTS
#define  LED_CAL_OUT                    LATA.LATA2  // xxxxxxx
#define  LED_ERROR_OUT              LATA.LATA3  // xxxxxxxxxx
#define SENSOR_POWER_OUT      LATA.LATA4  // xxxxxxx 
...
void interrupt(void)
{
    ...
    if (IOCAF3) { IOCAF3 = FALSE;}
    if (IOCAF4) { IOCAF4 = FALSE;}
    if (IOCAF5) { IOCAF5 = FALSE;}
    
 }
 
// EEPROM access  ---> unsigned long ?
EEPROM_write(unsigned char address, unsigned long new_EEPROM_value)
{
    unsigned long old_EEPROM_value = EEPROM_read(address);
...
unsigned long EEPROM_read(unsigned char address)
{
...
unsigned long measure_sensor()
{
    ...
    NO_ERROR();
    if (new_nr_pulses < cal_wet_pulses) nr_pulses = cal_wet_pulses;
    if (new_nr_pulses > cal_dry_pulses) nr_pulses = cal_dry_pulses;
    
    current_val = ((new_nr_pulses - cal_wet_pulses) << 10) / (cal_dry_pulses - cal_wet_pulses);
 ...
 void ERROR_LED(unsigned char color, unsigned char blink)
 {
    ...
    {
        LED_error_blink_cnt = ERROR_LED_BLINK_INTERVAL;
    }
 ...
 void init()
 {
    ...
    INTCON. IOCIE = 1;
    IOCAN3 = 1;                 // Falling edge PORTA.3 
    IOCAN4 = 1;                 // Falling edge PORTA.4
    IOCAN5 = 1;                 // Falling edge PORTA.5
    ...
    // Timer0 timebase  - / 256  ==> 512 micros
    OPTION_REG.PS0 = 1;
    OPTION_REG.PS1 = 1;
...
void main()
{
    ...
    while(1)
    {
        ...
        if (!PB_METER_WATER && PB_CAL_DRY && PB_CAL_WET)
        {
            ...
            old_water_value = EEPROM_Read (CURRENT_WATER_VALUE);
            ...
            EEPROM_write (CURRENT_WATER_VALUE, new_water_value);
